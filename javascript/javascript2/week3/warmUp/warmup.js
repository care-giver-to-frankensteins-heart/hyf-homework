...
//to log out the text: Called after 5 seconds
...
// the location (latitude, longitude) of the user
...
function jokeCreator(shouldTellFunnyJoke, logFunnyJoke, logBadJoke) {
  if (shouldTellFunnyJoke) pFunny.textContent = logFunnyJoke();
  else pBad.textContent = logBadJoke();
}
...
const arr = [
  function () {
    console.log(`this is function: 1 of ${arr.length}`);
  },
  function () {
    const x = 2;
    const y = 3;
...
const obj = {
  func1: function () {
    console.log(
      `this is function inside object: 1 of ${Object.keys(obj).length}`
    );
  },
  func2: function () {
    const x = 2;
